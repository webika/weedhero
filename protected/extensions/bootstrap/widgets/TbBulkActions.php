...
class TbBulkActions extends CComponent
{
	...
	public function registerClientScript()
	{
		...
		$js = <<<EOD
jQuery(document).on("click", "#{$this->grid->id} input[type=checkbox]", function(){
	var grid = jQuery("#{$this->grid->id}");
	if(jQuery("input[name='{$this->columnName}']:checked", grid).length)
	{

		jQuery(".bulk-actions-btn", grid).removeClass("disabled");
		jQuery("div.bulk-actions-blocker",grid).hide();
	}
	else
	{
		jQuery(".bulk-actions-btn", grid).addClass("disabled");
		jQuery("div.bulk-actions-blocker",grid).show();
	}
});
EOD;
		foreach ($this->events as $buttonId => $handler)
		{
			$js .= "\njQuery(document).on('click','#{$buttonId}', function(){var checked = jQuery('input[name=\"{$this->columnName}\"]:checked');\n
			var fn = $handler; if(jQuery.isFunction(fn)){fn(checked);}\nreturn false;});\n";
		}
