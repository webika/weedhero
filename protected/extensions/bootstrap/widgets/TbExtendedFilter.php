...
class TbExtendedFilter extends CWidget
{
	...
	public function registerClientScript()
	{
        ...
        jQuery(document).on('click', '#{$this->grid->id} .btn-extended-filter-save', function(e){
            e.preventDefault();
            bootbox.prompt("How do you wish to save this filter?", "Cancel", "Ok", function(result){
                if(jQuery.trim(result).length > 0)
                {
                    jQuery('#{$this->grid->id}').yiiGridView('update',{data:{{$this->saveFilterVar}:result}});
                }
            });
        });
        jQuery(document).on('click', '#{$this->grid->id} .btn-extended-filter-apply', function(e) {
            e.preventDefault();
            var option = jQuery('#{$this->getId()} select.select-extended-filter option:selected');
            ...
            jQuery('#{$this->grid->id}').yiiGridView('update',data);
        });

        jQuery(document).on('click', '#{$this->grid->id} .btn-extended-filter-delete', function(e) {
            e.preventDefault();
            var option = jQuery('#{$this->grid->id} select.select-extended-filter option:selected');
            ...
            bootbox.confirm('Delete "'+option.text()+'" filter?', function(confirmed){
                if(confirmed)
                {
                    jQuery('#{$this->grid->id}').yiiGridView('update',{data:{{$this->removeFilterVar}:option.data('key')}});
                }
            });
        });
EOD
